...
- **Protocol bridge**: `resources/page_bridge.js` is injected via CDP init script before game boot. It patches the game's own `Connection` to report all downstream packets as `ProtocolMessage` events; upstream sends go through `SessionCommand::SendProtocol` → `window.__wardenly.send(name, payload)`. The bridge also wraps `Connection.send` so client-originated upstream packets (e.g. a human clicking in the game UI) are reported too, flagged `self:false` — they go only to the session journal, never to GameState or runner waits. No binary/crypto is touched — the game encodes and decodes itself.
- **Session journal**: every session records its full protocol traffic (downstream + upstream, self-flagged) as JSONL at `logs/sessions/<timestamp>_<server>_<role>.jsonl` (`infrastructure/logging/journal.rs`). Best-effort: channel-full drops records instead of blocking the forwarder. Heartbeat/flood protocols are excluded. This is the recon tool for new tasks: play the feature manually, read the journal, write the template.
- **Unified task runner (recommended)**: `TaskRunner` executes schema-v2 tasks (`resources/tasks/*.yaml`) as a state-matching loop — template order is priority, `once: true` marks linear steps, `on_no_match` picks quit/wait. Predicates compose scene recognition with state./role. conditions; protocol and screenshot-fallback actions mix in one template. Payload `$`-references and array selectors (`@where/@max/@first`) are resolved at send/eval time; `@where` values may also be `$state.` references. `request` actions support `abort_if` (checked before send/each retry and polled mid-wait — aborts hand back to the state machine instantly when the request's premise expires, e.g. battle over). All task knowledge lives in YAML; the runner is task-agnostic. Legacy engines (`ScriptRunner`, `ProtocolRunner`) still serve `resources/scripts/` and `resources/protocols/`; tasks take priority in `start_script` routing.
- **JS library (jslib)**: reusable JS lives in `resources/jslib/*.js` (each defines `main(args)`), invoked from templates via `call_js {name, args, store}`. `store` merges the JSON return into GameState under `_js.<store>` for later `$state._js.*` references ("compute once, reference everywhere"). `eval_js` inline scripts are for one-off escape hatches only — never inline reusable JS in YAML.
