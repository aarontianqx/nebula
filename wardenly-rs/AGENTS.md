...
- **Login ready criterion**: `Connection._connected` AND bridge-observed `S_2_C_CHAR_LOAD_END` — the WS connects early, possibly while a canvas dialog still blocks game entry, so observed login traffic is the only reliable "server accepts business protocols" signal.
