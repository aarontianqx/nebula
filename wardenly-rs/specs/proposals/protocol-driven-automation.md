...
#### Phase 4 实现纪要（2026-09-02，以代码为准）

- 拟人化已落地前两层（在线时长封顶是产品决策，未做）：
  - **输入层**：`ChromiumDriver` 的 click/drag/drag_path 统一加 ±2px 落点抖动与 30–90ms 随机按住时长；
  - **节奏层**：`service/humanize.rs` 在每个脚本动作前加 150–450ms 随机延迟（两个 Runner 统一收口），staggered start 间隔从固定 1s 改为 0.5–2.0s 随机；
  - 均为横切运行时行为，YAML 无需任何改动，对未来脚本自动生效。
- 附带落地：登录后视口自检（非 1080×720 绘制区域时 warn，防止坐标系漂移后沉默点歪）。
- 验证：`login_chain` / `protocol_script` live 测试在拟人化开启后通过（19.2s / 14.3s）。
