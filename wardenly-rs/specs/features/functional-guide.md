...
桥同时包装了 `Connection.send`：**客户端自己发出的每条上行协议**（比如你手动点按钮时游戏发的包）也会被记录，写入事件流水（见下「事件流水」），不进 GameState、不参与脚本等待。自动化脚本发出的包带 `self: true` 标记，与游戏自发的区分开。

> 用途：手动玩一遍某个功能，流水里就是它的完整协议序列——"某个按钮到底发不发协议"看流水就知道。
...
每个会话从启动到关闭，全部协议流量按时间顺序写入独立的 JSONL 文件：

- 位置：`logs/sessions/<时间戳>_<区服>_<角色名>.jsonl`（数据目录下，见「数据目录」）
- 每条一行 JSON：`kind: down`（下行包）/ `kind: up`（上行发送，`self` 区分自动化与游戏自发）/ `kind: meta`（会话起止）
- 心跳、海上贸易等周期刷屏包已过滤

> 用途：排查问题时翻对应账号的流水即可还原现场；写新任务时手动玩一遍目标功能，流水就是协议参照。文件只增不删，长期跑可定期清理 sessions 目录。
...
- `default: <值>`：字段路径解析不出时用 default 替代参与比较（典型：购买计数条目在首次购买前不存在，default 0 = "已购 0"）。只应在"缺失有安全含义"时使用；不写 default 则缺失=条件不成立（保守方向）。
