...
> 已在 Phase 6 打通：**变量运行时替换**（M3，Resolve 阶段 + 运行前覆盖）、**子宏调用 `call_macro`**（M4，引擎就地展开 + 环/深度保护 + 子作用域）、**macOS 平台对齐**（M5，`CGWindowList` 窗口枚举 / `CGWindowListCreateImage` 像素取色 / 真实 `backingScaleFactor`，并统一坐标系：mac 用 point、Win 用物理像素，拾取器经 `browser_to_injection_scale` 换算）、**录制降噪**（M6，合成 Click/DoubleClick/Drag/KeyTap + 合并共线移动）、**拖拽插值与安全硬化**（M7，按 `duration_ms` 可中断插值且异常必抬起、注入看门狗超时、连续失败自动停、子秒级倒计时）、**前端重构与同步契约**（M8，Zustand store + 组件化；Timeline 编辑器 List/Rail/Code 三视图 + Inspector 参数编辑、增删/移动/复制/拖拽调时/批量调延时/note；编辑去抖回推 `update_profile` 且启动/保存前强制刷盘——彻底修复“前端编辑不落库”）、**Profile / 模板 / 运行前表单**（M9，编译期内嵌模板浏览 + 一键应用、最近使用列表、无损元数据编辑（描述/作者/标签）、运行前变量表单接入开始流程、原生文件对话框导入导出）、**权限检测与首次引导**（M10，启动/聚焦探测 macOS 辅助功能 + 屏幕录制权限并在横幅内引导授予、权限不足时禁用录制/回放/Key→Click、Windows 管理员提示、首次启动欢迎引导含紧急停止说明与示例宏一键加载）、**Key→Click 完善**（M11，可选鼠标按键 左/右/中、点击位置 跟随光标/固定坐标、`min_interval_ms` 限频与夹取、可选“锁定启动时活动窗口”、状态栏常驻运行提示与启动/停止原因日志）、**简单模式与安全增强**（M12，简单点击可选鼠标按键、按键“Capture”录入、试运行 Dry-run 开关（引擎 + Key→Click 仅记录不注入 + 顶栏徽标）、跨平台按键名归一化修复修饰键误注入、修复拾取器文案乱码）。
