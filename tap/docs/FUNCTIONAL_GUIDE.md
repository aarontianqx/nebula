...
- **安全停止**：全局热键（例如 `Ctrl+Shift+Esc`）立即停止
...
- 基于窗口/进程：仅当某窗口处于前台才执行
- 基于像素/截图：检测某区域颜色/图像匹配再点击（类似 wardenly 的 scene 概念）
- 简单分支：if/else，超时重试
- 变量与计数器：用于循环与退出条件

### 额外建议（更“有远见”的需求判断）

以下能力非常实用，且不会把 tap 变成“企业 RPA”，建议在第三阶段后逐步引入：

- **目标绑定**：宏绑定到特定窗口（标题/进程名/Bundle ID），运行时自动校验；不匹配则暂停并提示。
- **坐标空间**：支持“相对坐标”（相对窗口/相对 ROI）与分辨率缩放策略，提升跨设备稳定性。
- **随机抖动（可控）**：对点击坐标/延时做小范围随机（可配置），用于降低机械重复感（强调合法自用）。
...
支持用 JSON/YAML/自定义 DSL 描述宏（GUI 只是编辑器/可视化）：
...
长期维护的角度，建议按以下顺序演进（更易控、更可持续）：

1. **配置式 DSL（GUI 生成）**：覆盖 80% 的需求，降低脚本错误风险。
2. **内嵌轻脚本（可选）**：用于表达少量条件/计算（例如计数器、字符串拼接）。
3. **Wasm 插件**：用于扩展动作类型与复杂逻辑（同时保证隔离与可移植）。
