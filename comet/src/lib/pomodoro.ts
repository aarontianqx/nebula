...
function minutes(key: string, fallback: number): number {
...
function loadSession(): Session | null {
  try {
    ...
    if ((s.phase === "focus" || s.phase === "break") && s.endsAt > Date.now()) {
      return s;
    }
  } catch {
    // 损坏的会话数据直接丢弃
...
export class Pomodoro {
  ...
  toggle(): PomodoroPhase {
    if (this.session) {
      ...
      this.session = {
        phase: "focus",
        endsAt: Date.now() + minutes(FOCUS_MIN_KEY, DEFAULT_FOCUS_MIN) * 60_000,
      };
  ...
  private tick(): void {
    ...
    if (this.session.phase === "focus") {
      this.session = {
        phase: "break",
        endsAt: Date.now() + minutes(BREAK_MIN_KEY, DEFAULT_BREAK_MIN) * 60_000,
      };
      saveSession(this.session);
